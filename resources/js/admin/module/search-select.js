 ...
 * (2016.5.23)
 * 添加.J_delSelectTag的删除后额外响应事件的触发接口'deleteTagAfter'   by hj
 * ---------------------------------------------------------------------------*/
KISSY.add('module/search-select', function(S, Core){
    var
...
KISSY.add('select/core', function(S, TagEdit){
    ...
    S.augment(Plugin, S.EventTarget, {
        _init: function(){
            ...
            this.newElement = this._getOptions();
            this._createView();
            this._buildEvt();
        },
        _loadCSS: function(){
        ...
        _buildEvt: function(){
            ...
            delegate(document, 'click', '.render-list', function(e){
                e.stopPropagation();
                that._renderSearchResult(e.currentTarget);
            });

            delegate(document, 'click', '.J_delSelectTag', function(e){
                e.stopPropagation();
                that._delSelectTag(e.target);
                that.fire('deleteTagAfter');
            });

            if(that.opts.sendAjax){
                delegate(that.opts.root, 'keyup', el.inputEl, function(e){
                    that._sendAjax(DOM.val(e.currentTarget), e.currentTarget, that.opts.urlCoreParam);
                });
            }
         ...
         * @private
         */
        _selectMulLi: function(){
            var
                that = this,
                renderListTag = query('.select-tag', '.J_renderTo'),
                ulEl = get('ul', that.opts.root),
                lisEl = query('li', ulEl);
        ...
        unfold: function(){
            ...
            that._openMenu(that.currentTarget);
        },
        /**
         * 解绑事件
         * @private
         ...
         * @private
         */
        _renderSearchResult: function(e){
            var
                that = this,
                text = DOM.text(e),
                ...
                orderUnitName = DOM.attr(e, 'data-orderUnitName'),
                itemNumber = DOM.attr(e, 'data-itemNumber'),
                renderTo = get('.J_renderTo', that.opts.root);
            ...
            if(!that.opts.multiple){
                ...
                DOM.attr(renderTo, 'data-result-orderUnitName', orderUnitName);
                DOM.attr(renderTo, 'data-result-itemNumber', itemNumber);
                that._closeMenu();
                that.fire('selectAfter', {
                    extendParam: {
                        price: DOM.attr(renderTo, 'data-result-price'),
                        unit: DOM.attr(renderTo, 'data-result-unit'),
                        orderUnitName: DOM.attr(renderTo, 'data-result-orderUnitName'),
         ...
         * @private
         */
        _renderMul: function(activeLiArray){
            var
                that = this,
                text = '',
                renderListTag = query('.select-tag', '.J_renderTo'),
            ...
            if(that.opts.hasHiddenInput){
                for(var k = 0; k < mergeList.length; k ++){
                    text += '<div class="select-tag" data-result-id="' + mergeList[k].id +'" data-result-name="' + mergeList[k].name +'" data-result-price="' + mergeList[k].price +'" data-result-unit="' + mergeList[k].unit + '">' + mergeList[k].name +'<i class="fa fa-times J_delSelectTag"></i><input type="hidden" name="' + that.opts.inputName + '" value="' + mergeList[k].id + '" /></div>&nbsp;';
                }
            }else{
        ...
        _getRenderList: function(renderListTag){
            ...
            S.each(renderListTag, function(item){
                renderList.push({
                    ...
                    orderUnitName: DOM(item, 'orderUnitName')
                });
            });
            return renderList;
        },
         ...
         * @private
         */
        _sendAjax: function(text, e, urlCoreParam){
            var
                that = this,
        ...
        _renderList: function(list, e){
            ...
            S.each(list, function(it){
                lis += '<li class="render-list" data-price="' + it.price + '" data-name="' + it.name + '" data-id="' + it.id + '" data-code="' + it.assistantCode + '" data-unit="' + it.unit + '" data-orderUnitName="'+ it.orderUnitName +'" data-itemNumber="'+ it.itemNumber+'"><a href="javascript:;">'+ '[' + it.assistantCode + '] ' + it.name + '</a></li>';
            });
            listDOM = DOM.create(lis);
            searchBoxInput = get('.search-box-input', that.opts.root);
        ...
        _openMenu: function(e){
            ...
            searchInput = get(el.inputEl, that.opts.root);
            if(searchInput){
                searchInput.focus();
            }
        },
        ...
        _loadStyle: function(){
            var
                that = this;
            S.loadCSS(that.opts.selectStyle.themeUrl, {id: 'J_selectCSS'});
        },
        /**
        ...
        _getOptions: function(){
            var
                that = this,
                text,id,price,assistantCode, unit,orderUnitName,
                newElement = '<div class="drop clearfix">' +
                                '<div class="J_renderTo render-div clearfix">' +
            ...
            S.each($(that.$element).children(), function(item){
                ...
                orderUnitName = $(item).attr('data-orderUnitName');
                itemNumber = $(item).attr('data-itemNumber');
                newElement += '<li class="render-list" data-price="' + price + '" data-id="' + id + '" data-name="' + text + '" data-code="' + assistantCode + '" data-unit="' + unit + '" data-orderUnitName="' + orderUnitName + '" data-itemNumber="' + itemNumber +'"><a href="javascript:;">' + '[' + assistantCode + '] ' +  text + '</a></li>';
            });
            newElement += '</ul></div>';
            return newElement;
        },
        ...
        _createView: function(){
            ...
            DOM.append(newElementDOM, that.opts.root);
            ...
            // 如果带有输入框搜索,添加搜索输入框
            if(that.opts.liveSearch){
                var
                    searchBox = '<div class="search-box-input"><input type="text" class="form-control focusedInput J_focusedInput"><span class="fa fa-search search-box" aria-hidden="true"></span></div>';
                if(that.currentTarget == null || that.currentTarget == undefined){
                    that.currentTarget = get('.J_renderTo', that.opts.root);
                }
        ...
        _renderEditTag: function(){
            var
                that = this;

            new TagEdit(that.opts);
        },
        /**
         * 搜索时显示上次选中的项
         * @private
        ...
        _selectLi: function(){
            var
                that = this,
                ulEl = DOM.next(that.currentTarget, 'ul'),
                renderToEl = get('.J_renderTo', that.opts.root),
