...
KISSY.add('page/authority-management/authority-group-list', function(S, List){
...
KISSY.add('authority-config/list', function(S){
	var 
		...
		// 操作类型，编辑、添加
		OPER_TYPE = 'oper-type',
		TIP = ['确定删除该安全组吗？', '确定保存该安全组吗？', '确定添加该安全组吗？',
	...
	S.augment(List, {
		...
		_buildEvt: function(){
			...
			//查看权限
			delegate(document, 'click', el.searchTrigger, function(e){
				if(!that._hasEdit()){
					Dialog.alert(TIP[6]);
				};
			});
		...
		_del: function(e){
			...
			Dialog.confirm(TIP[0], function(){
				//发送id ajax
				AuthorityManagementIO.deleteAuthorityGroup({
					id: id
				}, function(rs, errMsg){
					if(rs){
						Dialog.alert(TIP[3]);
		...
		_saveEdit: function(data, tr){
			...
			Dialog.confirm(TIP[1], function(){
				//发ajax
				AuthorityManagementIO.saveAuthorityGroup(data, function(rs, errMsg){
					if(rs){
						Dialog.alert(TIP[4]);
		...
		_saveAdd: function(data){
			...
			Dialog.confirm(TIP[2], function(){
				//发ajax
				AuthorityManagementIO.saveNewAuthorityGroup(data, function(rs, errMsg){
					if(rs){
						Dialog.alert(TIP[5]);
						that.pagination.reload(that.opts);
					}else{
