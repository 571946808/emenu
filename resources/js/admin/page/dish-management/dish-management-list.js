...
KISSY.add('page/dish-management/dish-management-list', function(S, List){
...
KISSY.add('dish-management/list', function(S){
	var
		...
		DATA_DISH_STATUS = 'data-dish-status',
		newStatusMap = {
			0: '恢复',
			1: '停售'
		},
	...
	S.augment(List, {
		...
		_delDish: function(e){
			...
			Dialog.confirm('确定删除该菜品吗？', function(e,me){
				DishManagementIO.delDish({
					id: id
				},function(rs, errMsg){
					if(rs){
						Dialog.alert('删除成功！');
		...
		_changeStatus: function(e){
			...
			Dialog.confirm('确定' + newStatusMap[status] + '此菜品吗?', function(){
				DishManagementIO.changeDishStatus({
					id: id,
