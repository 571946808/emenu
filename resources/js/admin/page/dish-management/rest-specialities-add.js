...
* v1.0.0(2015.11.25):
     初始生成
* ---------------------------------------------------------------------------*/
KISSY.add('page/dish-management/rest-specialities-add', function(S, Add){
	PW.namespace('page.DishManagement.RestSpecialitiesAdd');
	PW.page.DishManagement.RestSpecialitiesAdd = function(param){
		new Add(param);
	}
}, {
	requires: [
		'rest-specialities/add'
	]
});
/*---------------------------------------------------------------------------*/
KISSY.add('rest-specialities/add', function(S){
	var
		DOM = S.DOM, get = DOM.get, query = DOM.query, $ = S.all,
		on = S.Event.on, delegate = S.Event.delegate,
		Dialog = PW.widget.Dialog,
		SelectAll = PW.mod.Selectall,
		RestSpecialitiesIO = PW.io.DishManagement,
	...
	S.augment(Add, {
		...
		_selectDishClassify: function(ev){
			...
			RestSpecialitiesIO.selectDishClassify(info, function(rs, list, errMsg){
				if(rs){
		...
		_addDish: function(){
			...
			RestSpecialitiesIO.sendSelectedDish(info, function(rs, errMsg){
				if(rs){
					Dialog.alert(TIP[0]);
				}else{
					Dialog.alert(TIP[1]);
	...
	requires: [
		...
		'mod/selectall',
		'mod/juicer',
		'pio/dish-management/rest-specialities'
	]
});