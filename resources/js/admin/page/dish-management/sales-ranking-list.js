...
* v1.0.0(2015.11.24):
     初始生成
* ---------------------------------------------------------------------------*/
KISSY.add('page/dish-management/sales-ranking-list', function(S, List){
	PW.namespace('page.DishManagement.SalesRankingList');
	PW.page.DishManagement.SalesRankingList = function(param){
		new List(param);
	}
}, {
	requires: [
...
/*---------------------------------------------------------------------------*/
KISSY.add('sales-ranking/list', function(S){
	var 
	...
	S.augment(List, {
		...
		_revokeDish: function(ev){
			...
			Dialog.confirm(TIP[0], function(){
				SalesRankingIO.revoteSales(info, function(rs,errMsg){
					if(rs){
						DOM.remove(tr);
						Dialog.alert(TIP[1]);
					}else{
