...
* v1.0.0(2015.11.25):
     初始生成
* ---------------------------------------------------------------------------*/
KISSY.add('page/dish-management/taday-specials-add', function(S, Add){
	PW.namespace('page.DishManagement.TodaySpecialsAdd');
	PW.page.DishManagement.TodaySpecialsAdd = function(param){
		new Add(param);
	}
}, {
	requires: [
...
/*---------------------------------------------------------------------------*/
KISSY.add('today-specials/add', function(S){
	var
	...
	S.augment(Add, {
		...
		_selectDishClassify: function(ev){
			...
			TodaySpecialsIO.selectDishClassify(info, function(rs, list, errMsg){
				if(rs){
		...
		_addDish: function(){
			...
			TodaySpecialsIO.sendSelectedDish(info, function(rs, errMsg){
				if(rs){
					Dialog.alert(TIP[0]);
				}else{
					Dialog.alert(TIP[1]);
