 ...
 * v1.0.0(2016.03.25):
 初始生成
 * ---------------------------------------------------------------------------*/
KISSY.add('page/recommend', function(S, Order, Core){
	PW.namespace('page.Recommend');
	PW.page.Recommend = function(param){
		new Core(param);
		new Order();
	}
},{
 ...
 * 顾客点“点餐”的相关操作
 */
KISSY.add('recommend/order', function(S, Common){
	var
		DOM = S.DOM, get = DOM.get, query = DOM.query,
		on = S.Event.on, delegate = S.Event.delegate,
		fire = S.Event.fire,
		RecommendIO = PW.io.Recommend,
		el = {
			//菜品列表
	...
	S.augment(Order, {
		...
		_addDish: function(e){
			...
			RecommendIO.sendDishInfo(info, function(rs,errMsg){
				if(rs){
 ...
 * 为您推荐的核心操作
 */
KISSY.add('recommend/core', function(S, Common){
	var
		DOM = S.DOM, get = DOM.get, query = DOM.query,
	...
	S.augment(Core, {
		...
		_load: function(){
			...
			//发送ajax
			RecommendIO.getRecommendDishList(info, function(rs, list, errMsg){
				if(rs){
 ...
 * 公共部分
 */
KISSY.add('recommend/common', function(S){
	var
		DOM = S.DOM,
		get = DOM.get,
		query = DOM.query,
