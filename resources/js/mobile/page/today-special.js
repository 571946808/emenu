 ...
 * v1.0.0(2016.06.02):
 初始生成
 * ---------------------------------------------------------------------------*/
KISSY.add('page/today-special', function(S, Order, Core){
	PW.namespace('page.TodaySpecial');
	PW.page.TodaySpecial = function(param){
		new Core(param);
		new Order();
	}
},{
 ...
 * 顾客点“点餐”的相关操作
 */
KISSY.add('today-special/order', function(S, Common){
	var
		DOM = S.DOM, get = DOM.get, query = DOM.query,
		on = S.Event.on, delegate = S.Event.delegate,
		fire = S.Event.fire,
		TodaySpecialIO = PW.io.TodaySpecial,
		el = {
			//菜品列表
	...
	S.augment(Order, {
		...
		_addDish: function(e){
			...
			TodaySpecialIO.sendDishInfo(info, function(rs,errMsg){
				if(rs){
 ...
 * 为您推荐的核心操作
 */
KISSY.add('today-special/core', function(S, Common){
	var
		DOM = S.DOM, get = DOM.get, query = DOM.query,
	...
	S.augment(Core, {
		...
		_load: function(){
			...
			//发送ajax
			TodaySpecialIO.getTodaySpecialDishList(info, function(rs, list, errMsg){
				if(rs){
 ...
 * 公共部分
 */
KISSY.add('today-special/common', function(S){
	var
		DOM = S.DOM,
		get = DOM.get,
		query = DOM.query,
